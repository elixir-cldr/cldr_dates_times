defmodule Cldr.DateTime.Relative do
  ...
  * `:derive_unit_from` is a map that is used to derive the time unit that best desribes the difference
  ...
  def to_string(relative, backend, options) do
    ...
    {derive_unit_from, options} = Keyword.pop(options, :derive_unit_from, @unit)
    ...
    with {:ok, locale} <- Cldr.validate_locale(locale, backend),
      ...
      {relative, unit} = define_unit_and_relative_time(relative, unit, relative_to, derive_unit_from)
  ...
  defp define_unit_and_relative_time(relative, nil, nil, derive_unit_from) do
    unit = unit_from_relative_time(relative, derive_unit_from)
    relative = scale_relative(relative, unit, derive_unit_from)
    {relative, unit}
  end

  # Take two datetimes and calculate the seconds between them
  defp define_unit_and_relative_time(relative, unit, relative_to, derive_unit_from)
    ...
    unit = unit || unit_from_relative_time(relative_time, derive_unit_from)
    relative = scale_relative(relative_time, unit, derive_unit_from)
    {relative, unit}
  end
  ...
  defp define_unit_and_relative_time(relative_time, unit, _relative_to, _derive_unit_from) do
    {relative_time, unit}
  end
  ...
  * `:derive_unit_from` is a map that is used to derive the time unit that best desribes the difference
  ...
  """
  def unit_from_relative_time(time, derive_unit_from \\ @unit)

  def unit_from_relative_time(time, derive_unit_from) when is_number(time) and is_map(derive_unit_from) do
    time= abs(time)

    cond do
      time < derive_unit_from[:minute] -> :second
      time < derive_unit_from[:hour] -> :minute
      time < derive_unit_from[:day] -> :hour
      time < derive_unit_from[:week] -> :day
      time < derive_unit_from[:month] -> :week
      time < derive_unit_from[:year] -> :month
      true -> :year
    end
  end

  # def unit_from_relative_time(time, _unit, _derive_unit_from) do
  #   time
  # end
  ...
  """
  def scale_relative(time, unit, derive_unit_from \\ @unit)
      when is_number(time) and is_atom(unit) and is_map(derive_unit_from) do
    (time / derive_unit_from[unit])
    |> Float.round()
    |> trunc
  end
