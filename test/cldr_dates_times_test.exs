defmodule Cldr.DatesTimes.Test do
  ...
  for year <- 0001..2200 do
    ...
    test "that we have either 52 or 53 weeks in the year starting #{inspect(starts)} ending #{
           inspect(ends)
         }" do
    ...
    test "that #{year} abuts the next year #{year + 1}" do
      assert Cldr.Calendar.next_day(
               Cldr.Calendar.ISOWeek.last_day_of_year(unquote(Macro.escape(year)))
